...
# Global variable to store curriculum data
curriculum_data = {}
...
def load_curriculum_data():
    """Load curriculum data from frontend request"""
    try:
        return {}
...
def set_curriculum_data():
    ...
    try:
        ...
        logger.info(f"Successfully loaded curriculum data for standards: {list(curriculum_data.keys())}")
        return jsonify({"message": "Curriculum data set successfully"})
        
    except Exception as e:
        logger.error(f"Error setting curriculum data: {str(e)}")
...
if __name__ == '__main__':
    ...
    if not curriculum_data:
        logger.warning("No curriculum data loaded. Waiting for frontend to provide data.")
    else:
